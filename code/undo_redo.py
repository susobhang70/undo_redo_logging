		...
		while(1):
			...
			for it in range(3):
				if command[it] is []:
					continue
				if startflag[it] == 0:
					t_undo += (log_transaction("start", variables, tnumber = it))
					t_redo += (log_transaction("start", variables, tnumber = it))
					startflag[it] = 1
				for i in command[it]:
					if 'READ' in i:
						i = i.replace('READ(', "")
						...
						t_undo = log_transaction("modify", variables, mvar = mvariables, t = t_undo, varname = tempvar_update, tnumber = it)
						mvariables[tempvar_update] = varval[it][var[it].index(tempvar_write)]
						t_redo = log_transaction("modify", variables, mvar = mvariables, t = t_redo, varname = tempvar_update, tnumber = it)
						# print t_undo

					elif 'OUTPUT' in i:
						if flag[it] == 0:
							t_undo = log_transaction("flushlog", variables, t = t_undo, fp = file_pointer_undo)
							t_redo = log_transaction("commit", variables, t = t_redo, tnumber = it)
							t_redo = log_transaction("flushlog", variables, t = t_redo, fp = file_pointer_redo)
						i = i.replace('OUTPUT(', "")
						tempvar_update_database = i[-2]
						variables[tempvar_update_database] = mvariables[tempvar_update_database]
						flag[it] += 1
						if flag[it] == 2:
							t_undo = log_transaction("commit", variables, t = t_undo, tnumber = it)
							t_undo = log_transaction("flushlog", variables, t = t_undo, fp = file_pointer_undo)
